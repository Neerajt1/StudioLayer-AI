...
// Desktop editorial (editorialAbsolute): original absolute-slot DOM (unchanged).
// Mobile / scroll rows: sl-talent-card--scroll layout for grid + collection.
// ---------------------------------------------------------------------------
...
export const TalentCard = memo(function TalentCard({
  ...
  if (editorialAbsolute) {
    return (
      ...
      >
        <div className="sl-talent-portrait-wrap relative min-h-0 flex-1">
          <ModelImageFrame
            src={model.imageUrl}
        ...
        <p
          className={cn(
            'sl-talent-name shrink-0 truncate text-[11px] font-medium leading-tight mt-2',
  ...
  return (
    <button
      ...
      className={cn(
        'sl-talent-card sl-talent-card--scroll group relative overflow-visible border-0 bg-transparent p-0 text-left select-none',
    ...
    >
      <div className="sl-talent-card-media relative flex w-full flex-col items-center">
        <div className="sl-talent-portrait-wrap relative w-full shrink-0">
          <ModelImageFrame
            src={model.imageUrl}
            alt={model.displayName}
            identityId={model.id}
        ...
        {!showSelected && !disabled && (
          <div
            className={cn(
              'sl-talent-select-hint pointer-events-none flex justify-center',
        ...
        {showSelected && (
          <div
            className={cn(
              'sl-talent-check pointer-events-none absolute top-0 right-0 z-[1] flex h-5 w-5 items-center justify-center rounded-full bg-foreground/90',
              'transition-transform duration-200 ease-out motion-reduce:transition-none',
              isSelecting && 'scale-110',
            )}
            aria-hidden
      ...
      <p
        className={cn(
          'sl-talent-name mt-4 w-full shrink-0 truncate text-[11px] font-medium leading-tight',
          'transition-colors duration-200 ease-out motion-reduce:transition-none',
          showSelected
            ? 'text-foreground'
            : 'text-foreground/65 group-hover:text-foreground motion-reduce:group-hover:text-foreground/65',
