...
export const ListRendersResponseItem = zod.object({
  ...
  "locationEnvironment": zod.enum(['photo_studio', 'white_studio', 'grey_gradient_studio', 'urban_street', 'luxury_interior', 'nature']),
...
export const CreateRenderBody = zod.object({
  ...
  "backImageUrl": zod.string().optional().describe('Optional back-view garment image. Supplementary; front (sourceImageUrl) remains primary.\n'),
  "detailImageUrl": zod.string().optional().describe('Optional detail/close-up garment image. Supplementary; front (sourceImageUrl) remains primary.\n'),
  ...
  "imageCount": zod.number().int().min(1).max(createRenderBodyImageCountMax).optional().describe('Number of output images to generate. Preset shoots: 1 (Hero), 2 (Editorial), or 4 (Campaign). When customCampaign is true, must be an integer from 4 to 20. Defaults to 1.\n'),
  "customCampaign": zod.boolean().optional().describe('When true, generates a Custom Campaign batch at the specified imageCount (4–20). Uses Campaign per-image pricing. Campaign preset (imageCount=4 without this flag) remains unchanged.\n'),
  "outputResolution": zod.enum(['2K', '4K']).optional().describe('Native output resolution selected before generation. 2K (default) costs 1.5 Studio Credits per successful image. 4K costs 3 Studio Credits per successful image. Refinements ignore this field and continue to cost 1 credit.\n'),
